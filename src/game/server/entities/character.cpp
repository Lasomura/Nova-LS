...
void CCharacter::DoWeaponSwitch()
{
	...
	if(m_ReloadTimer != 0 || m_QueuedWeapon == -1 || (!m_Telekines && m_Core.m_aWeapons[WEAPON_NINJA].m_Got) || !m_Core.m_aWeapons[m_QueuedWeapon].m_Got)
		return;
...
bool CCharacter::GiveWeapon(int Weapon, bool Remove, int Ammo)
{
	...
	else if(m_Core.m_aWeapons[Weapon].m_Ammo < g_pData->m_Weapons.m_aId[Weapon].m_Maxammo || !m_Core.m_aWeapons[Weapon].m_Got)
	{
		m_Core.m_aWeapons[Weapon].m_Ammo = Ammo;
		if(m_FreezeTime) //dont remove this
			Freeze(0); //dont remove this
		// m_Core.m_aWeapons[Weapon].m_Ammo = minimum(g_pData->m_Weapons.m_aId[Weapon].m_Maxammo, Ammo); // commented out by chiller
	}
	else
	{
		// TODO: make sure this is correct and make it less conflicting (chiller)
